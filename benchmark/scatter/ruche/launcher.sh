...
SCHEFILE=~/deisa-dask/benchmark/scatter/ruche/scheduler.json
DASK_WORKER_NODES=1
...
srun -N 1 -n 1 -c 1 -r 0 bash -c "source ~/venv3.14_deisa-dask/bin/activate && dask scheduler --scheduler-file=$SCHEFILE" &
dask_sch_pid=$!
...
srun -N ${DASK_WORKER_NODES} -n ${DASK_WORKER_NODES} -c 1 -r 1 bash -c "source ~/venv3.14_deisa-dask/bin/activate && dask worker \
