...
def plot_latency_curve(latency_data):
    ...
    for ds in sorted(by_ds_nc.keys()):
        ...
        plt.figure()
        plt.errorbar(clients, means, yerr=stds, marker="o")
        plt.yscale("log")
        plt.xlabel("Number of clients")
        plt.ylabel("Scatter latency (ns)")
        plt.title(f"Burst latency curve — data_size={ds}")
        plt.grid(True, which="both", alpha=0.3)
        plt.tight_layout()
        plt.savefig(f"plot_latency_curve_ds{ds}.png")
        plt.close()

        print(f"Saved plot_latency_curve_ds{ds}.png")
...
def plot_break_test(break_latency, break_success):
    ...
    for ds in sorted(break_latency.keys()):
        ...
        plt.figure()
        plt.errorbar(clients, lat_means, yerr=lat_stds, marker="o")
        plt.yscale("log")
        plt.xlabel("Number of clients")
        plt.ylabel("Mean scatter latency (ns)")
        plt.title(f"Break test latency — data_size={ds}")
        plt.grid(True, which="both", alpha=0.3)
        plt.tight_layout()
        plt.savefig(f"plot_break_latency_ds{ds}.png")
        plt.close()

        print(f"Saved plot_break_latency_ds{ds}.png")
        ...
        plt.figure()
        plt.plot(clients, success_means, marker="o")
        plt.xlabel("Number of clients")
        plt.ylabel("Mean successful scatters per client")
        plt.title(f"Scheduler survival curve — data_size={ds}")
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(f"plot_survival_curve_ds{ds}.png")
        plt.close()

        print(f"Saved plot_survival_curve_ds{ds}.png")
